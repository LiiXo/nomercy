...
const MatchSheet = () => {
  ...
  // Create voice channels (admin/staff only)
  const handleCreateVoiceChannels = async () => {
    if (!isRankedMatch || !isStaff || creatingVoiceChannels) return;
    setCreatingVoiceChannels(true);
    try {
      const response = await fetch(`${API_URL}/ranked-matches/admin/${matchId}/create-voice-channels`, {
        method: 'POST',
        credentials: 'include'
      });
      const data = await response.json();
      if (data.success) {
        // Rafraîchir les données du match pour afficher les nouveaux salons vocaux
        fetchMatchData(false);
        console.log('[MatchSheet] Voice channels created successfully:', data.team1VoiceChannel, data.team2VoiceChannel);
      } else {
        alert(data.message || 'Erreur lors de la création des salons vocaux');
        console.error('Create voice channels error:', data.message);
      }
    } catch (err) {
      console.error('Error creating voice channels:', err);
      alert('Erreur lors de la création des salons vocaux');
    } finally {
      setCreatingVoiceChannels(false);
    }
  };
            ...
            {isStaff && ['accepted', 'in_progress', 'ready'].includes(match.status) && (
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  ...
                  {/* Générer les salons vocaux Discord - Uniquement pour les matchs classés */}
                  {isRankedMatch && (
                    <button
                      onClick={handleCreateVoiceChannels}
                      disabled={creatingVoiceChannels}
                      className="w-full py-2 px-3 bg-indigo-500/20 border border-indigo-500/40 rounded-lg text-indigo-400 text-sm font-medium hover:bg-indigo-500/30 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {creatingVoiceChannels ? (
                        ...
                        <>
                          ...
                          {language === 'fr' ? '🎤 Générer salons vocaux Discord' : '🎤 Generate Discord Voice Channels'}
                        </>
                      )}
                    </button>
                  )}
