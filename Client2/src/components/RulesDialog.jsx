      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="hud-panel relative min-h-[50vh] max-h-[85vh] overflow-hidden">
                {/* Top border accent */}
                ...
                <div className="p-8 overflow-y-auto max-h-[85vh]">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-mono text-gray-500 uppercase tracking-wider">Game Rules</span>
                    </div>
                    <h2 className="text-2xl font-mono font-bold text-white uppercase tracking-wide">{currentRules.title}</h2>
                  </div>
                  ...
                  <div className="space-y-2 mb-8">
                    {currentRules.rules.map((rule, index) => (
                      <motion.div
                        ...
                        className="flex items-start gap-3 p-3 bg-white/5 border-l-2 border-accent-primary/30"
                      >
                        <span className="flex-shrink-0 text-[10px] font-mono text-accent-primary">
                          [{String(index + 1).padStart(2, '0')}]
                        </span>
                        <p className="text-sm text-gray-300 font-light">{rule}</p>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  <button
                    onClick={handleClose}
                    className="w-full py-3 bg-white/5 hover:bg-accent-primary/20 text-white font-mono text-xs uppercase tracking-wider border border-white/10 hover:border-accent-primary/30 transition-all"
                  >
                    [ {t('understood')} ]
                  </button>
                </div>
