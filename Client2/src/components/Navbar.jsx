    ...
    <>
      ...
      >
        <div className="px-4 md:px-8 lg:px-16 py-2">
          <div className="hud-panel px-4 md:px-6 py-2 flex items-center justify-between">
            {/* Logo */}
              ...
              {navItems.map((item) => {
                ...
                if (item.disabled) {
                  return (
                    ...
                    >
                      <span className="relative z-10 flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-gray-500">
                        <span className="text-[10px] opacity-50">{item.icon}</span>
                        {item.label}
                        {item.badge && (
                          <span className="px-1.5 py-0.5 text-[8px] bg-gradient-to-r from-amber-500 to-orange-500 text-black font-bold rounded-full">
                            {item.badge}
                          </span>
                        )}
                      </span>
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`relative z-10 flex items-center gap-2 text-xs font-mono uppercase tracking-wider ${
                      isActive ? 'text-accent-primary' : 'text-gray-500 group-hover:text-white'
                    } transition-colors`}>
                      <span className="text-[10px] opacity-50">{item.icon}</span>
                      {item.label}
                      {item.badge && (
            ...
            <div ref={searchRef} className="relative hidden md:block">
              <div className="relative">
                <input
                  ...
                  className="w-48 lg:w-56 px-4 py-1.5 pl-9 bg-black/30 border border-white/10 text-xs font-mono text-white placeholder-gray-600 focus:outline-none focus:border-accent-primary/50 transition-colors"
                />
              ...
              <AnimatePresence>
                {showSearchResults && searchResults.length > 0 && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="absolute top-full left-0 right-0 mt-2 py-2 bg-gradient-to-b from-dark-600 to-dark-700 rounded-xl border border-accent-primary/20 shadow-xl shadow-accent-primary/10 overflow-hidden z-50"
                  >
                    {searchResults.map((player) => (
                      ...
                      >
                        <div className="w-8 h-8 rounded-full overflow-hidden bg-dark-500 flex-shrink-0">
                          {getPlayerAvatarUrl(player) ? (
                            <img src={getPlayerAvatarUrl(player)} alt={player.username} className="w-full h-full object-cover" />
                          ) : (
                            <span className="w-full h-full flex items-center justify-center text-accent-primary text-xs font-medium">
            ...
            <div className="flex items-center gap-4">
              ...
              <div ref={soundMenuRef} className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-br from-dark-500 to-dark-600 border transition-colors text-sm font-medium ${
                    soundEnabled 
                ...
                <AnimatePresence>
                  {showSoundMenu && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute right-0 mt-2 py-3 px-4 w-48 bg-gradient-to-b from-dark-600 to-dark-700 rounded-xl border border-accent-primary/20 shadow-xl shadow-accent-primary/10 z-50"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-sm text-white">Sons UI</span>
                        <button
                          onClick={() => {
                      ...
                      <div>
                        <span className="text-xs text-gray-400 block mb-2">Volume: {Math.round(volume * 100)}%</span>
                        <input
                          type="range"
                          min="0"
                          max="1"
                          ...
                          className="w-full h-1 bg-dark-500 rounded-full appearance-none cursor-pointer accent-accent-primary"
                        />
                      </div>
                    </motion.div>
                  )}
              ...
              <div ref={langMenuRef} className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-br from-dark-500 to-dark-600 border border-accent-primary/10 hover:border-accent-primary/30 transition-colors text-sm font-medium text-white"
                >
                  <img 
                ...
                {showLangMenu && (
                  <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="absolute right-0 mt-2 py-2 w-36 bg-gradient-to-b from-dark-600 to-dark-700 rounded-xl border border-accent-primary/20 shadow-xl shadow-accent-primary/10"
                  >
                    {availableLanguages.map((lang) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2 text-sm hover:bg-accent-primary/10 transition-colors ${
              ...
              {isAuthenticated ? (
                <div className="flex items-center gap-3">
                  <Link to="/profile" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                    <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br from-accent-primary/30 to-accent-secondary/20 border border-accent-primary/30">
                      {getAvatarUrl() ? (
                    ...
                    <span className="text-sm text-white font-medium">{user?.username || user?.discordUsername}</span>
                  </Link>
                  <button
                    onClick={logout}
                ...
                <motion.button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-[#FF6B35] hover:bg-[#E85A2A] text-white font-semibold rounded-xl transition-all duration-300"
                >
                  {t('login')}
                </motion.button>
              )}
